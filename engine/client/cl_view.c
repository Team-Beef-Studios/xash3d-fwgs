...
static void V_AdjustFov( float *fov_x, float *fov_y, float width, float height, qboolean lock_x )
{
	...
	*fov_x = V_CalcFov( &y, height, width );
	...
	// workaround for motion control zooming issue
	bool zoomed = 90.0f / *fov_y > 1.1f;
	bool superzoomed = 90.0f / *fov_y > 5.0f;
	if (superzoomed && (Cvar_VariableValue("vr_zoom_by_motion") > 0.5f)) {
		Cvar_SetValue("vr_zoomfix", 1);
		*fov_x = 90;
		*fov_y = 90;
	} else {
		Cvar_SetValue("vr_zoomfix", 0);
	}

	*fov_x *= VR_GetConfigFloat(VR_CONFIG_VIEWPORT_FOVX) / 90.0f;
	*fov_y *= VR_GetConfigFloat(VR_CONFIG_VIEWPORT_FOVY) / 90.0f;
	Cvar_LazySet("vr_superzoomed", superzoomed ? 1 : 0);
	Cvar_LazySet("vr_zoomed", zoomed ? 1 : 0);
}
